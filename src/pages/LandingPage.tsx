...
function Hero() {
  ...
  return (
    <section className="relative overflow-hidden bg-gradient-to-b from-volt-100/60 via-volt-50/40 to-volt-100/50">
      <div className="absolute inset-0 grid-map opacity-40 mix-blend-overlay" />
      <div className="absolute -right-40 -top-40 h-[36rem] w-[36rem] rounded-full bg-volt-400/20 blur-3xl" />
      <div className="absolute -left-40 top-40 h-[28rem] w-[28rem] rounded-full bg-emerald-300/20 blur-3xl" />
      ...
      <div className="container-x relative grid items-center gap-10 pt-8 pb-16 lg:grid-cols-[1.05fr_1fr] lg:pt-12 lg:pb-24">
        <div className="animate-spring-up">
          <span className="chip-volt mb-4 ring-1 ring-volt-200">
          ...
          <div className="mt-8 flex items-center gap-6 text-sm text-ink-500">
            ...
            <div className="flex items-center gap-1.5"><Zap className="h-4 w-4 text-volt-500" /> Real-time availability</div>
          </div>
        </div>

        <div className="relative animate-scale-in group">
          <div className="relative overflow-hidden rounded-4xl shadow-map ring-1 ring-ink-200 transition-all duration-500 hover:scale-[1.02] hover:shadow-2xl animate-float-slow">
            <img src={HERO_IMG} alt="EV charging station" className="h-[420px] w-full object-cover transition-transform duration-700 group-hover:scale-110 sm:h-[480px]" loading="eager" />
            <div className="absolute inset-0 bg-gradient-to-t from-ink-900/70 via-ink-900/10 to-transparent" />
            <div className="absolute left-4 top-4 flex items-center gap-2 rounded-xl bg-white/90 px-3 py-2 backdrop-blur animate-float" style={{ animationDelay: '1s' }}>
          ...
          <div className="absolute -right-4 top-8 hidden rounded-2xl bg-white p-3 shadow-card ring-1 ring-ink-200 sm:block animate-glow-pulse">
            <div className="flex items-center gap-2">
              <span className="grid h-9 w-9 place-items-center rounded-xl bg-volt-50 text-volt-600"><BatteryCharging className="h-5 w-5" /></span>
              <div>
      ...
      <div className="container-x grid grid-cols-2 gap-px py-10 md:grid-cols-4">
        {stats.map((s) => (
          <div key={s.label} className="px-4 text-center">
            ...
            <p className="mt-1 text-sm font-semibold text-volt-100">{s.label}</p>
          </div>
        ))}
      </div>
    </section>
...
function HowItWorks() {
  const steps = [
    { icon: Search, title: 'Find a Charger', desc: 'Search by location or let the map show nearby stations with live availability.', color: 'text-volt-600 bg-volt-50' },
    ...
    { icon: BatteryCharging, title: 'Reserve or Start Charging', desc: 'Book a slot in advance or plug in and start your session instantly.', color: 'text-volt-600 bg-volt-50' },
    { icon: Wallet, title: 'Pay Securely', desc: 'Use your VoltGo wallet, UPI or cards. Transparent pricing, no surprises.', color: 'text-volt-600 bg-volt-50' },
  ];
  return (
    <section className="container-x py-20">
      ...
      <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
        {steps.map((s, i) => (
          <div key={s.title} className="card-pad relative transition-all duration-300 hover:-translate-y-3 hover:scale-110 hover:shadow-2xl cursor-default">
...
function WhyVoltGo() {
  const features = [
    ...
    { icon: Wallet, title: 'Transparent pricing', desc: 'See ₹/kWh, parking, idle and reservation fees upfront. What you see is what you pay.', color: 'text-volt-600 bg-volt-50' },
    { icon: Plug, title: 'Multiple charger types', desc: 'CCS2, Type 2, CHAdeMO and GB/T — filter stations by your vehicle\'s connector.', color: 'text-volt-600 bg-volt-50' },
    { icon: ShieldCheck, title: 'Secure digital wallet', desc: 'UPI, cards and net banking in one wallet. Recharge once, charge anywhere.', color: 'text-volt-600 bg-volt-50' },
    ...
    { icon: History, title: 'Charging history', desc: 'Every session logged with energy, duration, cost and downloadable invoices.', color: 'text-volt-600 bg-volt-50' },
  ];
  return (
    <section className="bg-gradient-to-b from-white to-volt-50/50 py-20">
      <div className="container-x">
        ...
        <div className="mt-12 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {features.map((f) => (
            <div key={f.title} className="card-pad group transition-all duration-300 hover:-translate-y-3 hover:scale-110 hover:shadow-2xl cursor-default">
      ...
      <div className="mt-10 grid gap-5 md:grid-cols-3">
        {featured.map((s) => (
          <button
            ...
            className="card group overflow-hidden p-0 text-left transition-all duration-300 hover:-translate-y-3 hover:scale-[1.08] hover:shadow-2xl"
          >
            <div className="relative h-40 overflow-hidden bg-gradient-to-br from-volt-100 to-volt-200/50 grid-map transition-transform duration-500 group-hover:scale-110">
    ...
    <section className="bg-ink-900 py-20 text-white">
      <div className="container-x">
        ...
        <div className="mt-10 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {items.map((c) => (
            <div key={c.name} className="rounded-2xl bg-white/5 p-5 ring-1 ring-white/10 transition-all duration-300 hover:-translate-y-3 hover:scale-110 hover:bg-white/10 hover:shadow-2xl cursor-default">
...
function CTA() {
  ...
  return (
    <section className="container-x py-20">
      <div className="relative overflow-hidden rounded-4xl bg-gradient-to-br from-volt-500 to-emerald-600 px-6 py-14 text-center text-white sm:px-12 shadow-2xl">
